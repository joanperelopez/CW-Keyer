...
Recent Update History
    ...
    2025 - Nov. 7
      Pere: activated TX1 and TX2 simultaneous
...
#define CODE_VERSION "2024.09.03.2239"

#define eeprom_magic_number 51               // you can change this number to have the unit re-initialize EEPROM

#include <stdio.h>
#include "keyer_hardware.h"
#include "keyer_pere_modif.h"                //LEDs Pere EA3AGK's modif.
  ...
  if (ptt_line_activated == 0) {   // if PTT is currently deactivated, bring it up and insert PTT lead time delay
        ...
        if (current_tx_ptt_line && (configuration.ptt_buffer_hold_active || manual_ptt_invoke_ptt_input_pin)) {
            ...
            if (winkey_pinconfig_ptt_bit){
              digitalWrite (configuration.current_ptt_line, ptt_line_active_state);   // Pere
...
void ptt_unkey(){
  ...
  if (ptt_line_activated) {
      ...
      if (current_tx_ptt_line) {
        digitalWrite (current_tx_ptt_line, ptt_line_inactive_state);  // Pere
...
void tx_and_sidetone_key (int state)
{
    ...
    if ((state) && (key_state == 0)) {
      if (key_tx) {
        ...
        if (current_tx_key_line) {
                                  digitalWrite (current_tx_key_line, tx_key_line_active_state);   // Modif Pere para activar ambos puertos de salida
                                  digitalWrite (tx_key_line_2, tx_key_line_active_state);
                                  }
      ...
      if ((state == 0) && (key_state)) {
        if (key_tx) {
          if (current_tx_key_line) {
                                    digitalWrite (current_tx_key_line, tx_key_line_inactive_state); // Modif Pere
    ...
    if ((state) && (key_state == 0)) {
      if (key_tx) {
        ...
        if (current_tx_key_line) {
                                  digitalWrite (current_tx_key_line, tx_key_line_active_state);  // Modif Pere
                                  digitalWrite (tx_key_line_2, tx_key_line_active_state);
                                  }
        #if defined(OPTION_WINKEY_2_SUPPORT) && defined(FEATURE_WINKEY_EMULATION) && !defined(FEATURE_SO2R_BASE)
          if ((wk2_both_tx_activated) && (tx_key_line_2)) {
...
void initialize_display(){
     ...
     if (LCD_COLUMNS < 9) {
      ...
      lcd_center_print_timed("K3NG - K5BCQ Keyer", 0, 4000);
        ...
        if (LCD_ROWS == 2) {
          lcd_center_print_timed(custom_startup_field, 1, 4000);    // display the custom field on the second line of the display, maximum field length is the number of columns
	        ...
	        lcd_center_print_timed("Happy CW!", 1, 4000);                    // display 'hi' on the 2nd line anyway
      ...
      #else
        lcd_center_print_timed("Happy CW!", 1, 4000);
